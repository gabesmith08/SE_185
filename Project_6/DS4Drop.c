...
int main(void) {
	...
	while (((mag(xVar, yVar, zVar)) > 0.30) && ((mag(xVar, yVar, zVar)) < 1.1)) { 
		...
		vNew = vOld + (mag(xVar, yVar, zVar) * 9.8) * (tNew - tOld);
		xNew = xOld + vNew * (tNew - tOld);
		vOld = vNew;
		xOld = xNew;
		tOld = tNew;
		tCurrent = t;
		tNew = (tCurrent - tRest) / 1000;
	...
	x1 = (x0 + (v * tFall) + 0.5 * g * (tFall * tFall));
		...
		printf("Compensating for air resistance, the fall was %6.3lf meters.\n", xNew);
	
	double finalPercent = (1 - (xNew / x1)) * 100;
	
		printf("this is %1.0lf%% less than computed before.",	finalPercent);
